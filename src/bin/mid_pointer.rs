...
fn main() -> io::Result<()> {
    ...
    let mut input: Vec<&str> = input.iter().map(|s| &**s).collect();
    let input = input.as_mut_slice();

    alphanumeric_sort::sort_str_slice(input);
    let input: Vec<String> = input.iter().map(|f| f.to_string()).collect();

    let mut midpoint_iterator = Some(MidpointIterator::new(input, options.initial_lod));
    let mut lods: Vec<Vec<String>> = Vec::new();
    // println!("lods: {:?}", midpoint_iterator.get_lods());
    loop {
        if let Some(smi) = midpoint_iterator {
            lods.push(smi.clone().collect());
            midpoint_iterator = smi.next_lod();
        } else {
            break;
        }
    }
    let mut lods_out: Output = Vec::new();
    lods_out.push(lods.first().unwrap().clone());
    for lod in &lods[1..] {
        let out: Vec<String> = lod
            .iter()
            .filter(|e| !lods_out.last().unwrap().contains(e))
            .cloned()
            .collect();
        lods_out.push(out);
    }

    let output: String = serde_json::to_string_pretty(&lods_out)?;
